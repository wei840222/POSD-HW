...
class DFSIterator : public Iterator<T> {
 public:
  ...
  void first() {
    while (!_iteratorStack.empty()) _iteratorStack.pop();
    ...
    if (!isDone()) {
      if (isLeaf()) {
        ...
        while (!isDone() && _iteratorStack.top()->isDone()) {
          _iteratorStack.pop();
          if (!isDone()) _iteratorStack.top()->next();
        }
      } else
...
class BFSIterator : public Iterator<T> {
 public:
  ...
  void first() {
    while (!_iteratorQueue.empty()) _iteratorQueue.pop();
  ...
  void next() {
    if (!isDone()) {
      if (isLeaf()) {
        ...
        if (_iteratorQueue.front()->isDone()) _iteratorQueue.pop();
      } else {
